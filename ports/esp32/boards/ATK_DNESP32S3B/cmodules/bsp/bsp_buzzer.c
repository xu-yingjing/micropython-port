...
typedef struct _bsp_buzzer_obj_t {
    mp_obj_base_t base;
    mp_obj_t pin;
    mp_int_t active;
} bsp_buzzer_obj_t;

static mp_obj_t bsp_buzzer_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *args) {
    ...
    bsp_buzzer_obj_t *self = mp_obj_malloc(bsp_buzzer_obj_t, type);
...
static mp_obj_t buzzer_on(mp_obj_t self_in) {
    bsp_buzzer_obj_t *self = (bsp_buzzer_obj_t *)MP_OBJ_TO_PTR(self_in);
...
MP_DEFINE_CONST_FUN_OBJ_1(bsp_buzzer_on, buzzer_on);

static mp_obj_t buzzer_off(mp_obj_t self_in) {
    bsp_buzzer_obj_t *self = (bsp_buzzer_obj_t *)MP_OBJ_TO_PTR(self_in);
    mp_pin_p_t *pin_p = (mp_pin_p_t *)MP_OBJ_TYPE_GET_SLOT(mp_obj_get_type(self->pin), protocol);
...
MP_DEFINE_CONST_FUN_OBJ_1(bsp_buzzer_off, buzzer_off);

static mp_obj_t buzzer_toggle(mp_obj_t self_in) {
    bsp_buzzer_obj_t *self = (bsp_buzzer_obj_t *)MP_OBJ_TO_PTR(self_in);
    mp_pin_p_t *pin_p = (mp_pin_p_t *)MP_OBJ_TYPE_GET_SLOT(mp_obj_get_type(self->pin), protocol);
...
MP_DEFINE_CONST_FUN_OBJ_1(bsp_buzzer_toggle, buzzer_toggle);

static const mp_rom_map_elem_t bsp_buzzer_locals_dict_table[] = {
    { MP_ROM_QSTR(MP_QSTR_on), MP_ROM_PTR(&bsp_buzzer_on) },
    { MP_ROM_QSTR(MP_QSTR_off), MP_ROM_PTR(&bsp_buzzer_off) },
    { MP_ROM_QSTR(MP_QSTR_toggle), MP_ROM_PTR(&bsp_buzzer_toggle) },
};
static MP_DEFINE_CONST_DICT(bsp_buzzer_locals_dict, bsp_buzzer_locals_dict_table);

MP_DEFINE_CONST_OBJ_TYPE(
    bsp_buzzer_type,
    MP_QSTR_BUZZER,
    MP_TYPE_FLAG_NONE,
    make_new, bsp_buzzer_make_new,
    locals_dict, &bsp_buzzer_locals_dict
    );
